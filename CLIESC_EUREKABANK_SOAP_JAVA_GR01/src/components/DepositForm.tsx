import z from "zod";
import { DepositRequest } from "../dto/transaction";
...
interface Props {
  ...
  onSubmit?: (dto: DepositRequest) => Promise<void> | void;
...
export function DepositForm({ submitError, onSubmit, onCancel }: Props) {
  ...
  return (
    ...
    >
      <h2 className="text-xl font-semibold text-white flex gap-x-2 items-center">
        <span
          className={`w-8 h-8 rounded-full content-center ${"text-green-500 bg-green-900/50"}`}
        >
