...
public class ParameterService {
    ...
    public List<ParameterDTO> getAllParameters() {
        return parameterRepository.findAll().stream().map(this::toDTO).collect(Collectors.toList());
    }

    public ParameterDTO getByKey(String key) {
        if (key == null || key.isBlank()) throw new ParameterException("Parameter key is required", 400);
        return toDTO(parameterRepository.findByKey(key.trim())
    ...
    public ParameterDTO updateParameter(Long id, ParameterDTO request) {
        if (id == null) throw new ParameterException("Parameter id is required", 400);
        ...
        parameter.setValue(request.getValue().trim());
        parameter.setDescription(normalize(request.getDescription()));
        return toDTO(parameterRepository.update(parameter));
    }
    ...
    public String getValueOrDefault(String key, String defaultValue) {
        return parameterRepository.findByKey(key).map(Parameter::getValue).orElse(defaultValue);
    }
    ...
    private void validateRequest(ParameterDTO request) {
        ...
        if (request.getKey() == null || request.getKey().isBlank()) throw new ParameterException("Parameter key is required", 400);
        if (request.getValue() == null || request.getValue().isBlank()) throw new ParameterException("Parameter value is required", 400);
    }

    private ParameterDTO toDTO(Parameter p) {
        return new ParameterDTO(p.getId(), p.getKey(), p.getValue(), p.getDescription());
    }
