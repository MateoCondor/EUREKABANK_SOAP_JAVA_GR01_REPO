...
public class AccountController {
    ...
    /**
     * Endpoint interno para MS_TRANSACCIONES.
     * Aplica un delta al saldo (positivo=crédito, negativo=débito).
     * No expuesto en el CORE gateway, solo para comunicación inter-microservicio.
     *
    ...
    @PUT
    @Path("/{id}/balance")
    public Response updateBalance(@PathParam("id") Long id, BalanceDeltaDTO request) {
        if (request == null || request.getDelta() == null) {
            return Response.status(400).entity(Map.of("message", "delta is required")).build();
        }
        AccountResponseDTO updated = accountService.applyBalanceDelta(id, request.getDelta());
        return Response.ok(updated).build();
    }
}
